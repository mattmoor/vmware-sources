...
func (ass *VSphereSourceStatus) MarkServiceUnavailable(name string) {
	condSet.Manage(ass).MarkFalse(
		VSphereSourceConditionReady,
		"ServiceUnavailable",
		"Service %q wasn't found.", name)
}

func (ass *VSphereSourceStatus) MarkServiceAvailable() {
	condSet.Manage(ass).MarkTrue(VSphereSourceConditionReady)
}
