...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.VSphereSources(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.VSphereSource, desired *v1alpha1.VSphereSource) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.SamplesV1alpha1().VSphereSources(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.SamplesV1alpha1().VSphereSources(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.VSphereSource) (*v1alpha1.VSphereSource, error) {
	...
	actual, err := r.Lister.VSphereSources(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.SamplesV1alpha1().VSphereSources(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
